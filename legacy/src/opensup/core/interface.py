...
Top-level orchestrator for the PGS encoding pipeline.

Owns the full encode workflow: loading BDN XML, assigning
palettes, coordinating per-event encoding across workers,
and writing the final .sup file. Exposes BDNRender as the
main public API consumed by both CLI and GUI.

Why this module: centralises pipeline orchestration so that
individual core components (segments, renderer, filestreams)
remain independent and testable in isolation.
"""
...
try:
    ...
    LayoutEngine = None  # type: ignore[assignment]
    Brule = None  # type: ignore[assignment]
    logger.warning("brule library not found. Install via: pip install brule")
from opensup.utils.geometry import Box
from opensup.media.pgraphics import PGDecoder
from opensup.core.filestreams import BDNXML, BDNXMLEvent, remove_dupes, add_periodic_refreshes
from opensup.core.segments import Epoch, DisplaySet
from opensup.media.optimizer import Quantizer
from opensup.media.pgstream import is_compliant, check_pts_dts_sanity, test_rx_bitrate, EpochContext, debug_stats
from opensup.core.renderer import PaddingEngine, EpochEncoder
...
def _pool_worker_init():
    if os.name == 'nt':
        try:
            ...
            # psutil no disponible en esta instalacion de Windows
            return
...
class BDNRender:
    ...
    def prepare(self) -> BDNXML:
        ...
        logger.debug(f"Parameters: {stkw}")

        ignore_res = self.kwargs.get('ignore_resolution', False)
        bdn = BDNXML(os.path.expanduser(self.bdn_file), ignore_resolution=ignore_res)
        fps_str = int(bdn.fps) if float(bdn.fps).is_integer() else round(float(bdn.fps), 3)
        fmt_str = f"{bdn.width}x{bdn.height}"
        logger.iinfo(f"BDN metadata: {fmt_str}, FPS={fps_str}, DF={bdn.dropframe}, {len(bdn.events)} valid events.")
    ...
    def find_all_layouts(self, bdn: BDNXML) -> list[EpochContext]:
        if LayoutEngine is None:
            raise ImportError(
                "brule library is required for layout analysis. "
                "Install with: pip install brule"
            )
    ...
    def _convert_single(self, bdn: BDNXML) -> None:
        ...
        _abort_ev = getattr(self, '_abort_event', None)
        for ectx in epochs_ctx:
            if _abort_ev is not None and _abort_ev.is_set():
                logger.warning("Abort requested, stopping epoch encoding.")
                break
    ...
    def _convert_mt(self, bdn: BDNXML) -> None:
        ...
        # Check abort after layout analysis (long operation)
        _abort_ev = getattr(self, '_abort_event', None)
        if _abort_ev is not None and _abort_ev.is_set():
            logger.warning("Abort requested after layout analysis, stopping before worker start.")
            self._epochs = []
            return
        ...
        while not all(map(lambda worker: worker.is_available(), workers)):
            if _abort_ev is not None and _abort_ev.is_set():
                logger.warning("Abort requested while waiting for workers, stopping.")
                self._epochs = []
                return
            time.sleep(0.2)
        ...
        while group_data is not None and healthy:
            if _abort_ev is not None and _abort_ev.is_set():
                logger.warning("Abort requested, stopping epoch distribution.")
                break
            time.sleep(0.05)
            ...
            for free_worker in filter(lambda worker: busy_flags[worker.iid] or worker.is_available(), workers):
                ...
                if not busy_flags[free_worker.iid] or not free_worker.is_alive():
                    if free_worker.is_alive():
                        ...
                        busy_flags[free_worker.iid] = False
                    logger.debug(f"Worker {free_worker.iid} closed.")
        ...
        if healthy and not (_abort_ev is not None and _abort_ev.is_set()):
            ...
            logger.warning("Abort: cleaning up workers.")
        time.sleep(0.01)
        for worker in workers:
        ...
        self._workers.clear()
        if not healthy and not (_abort_ev is not None and _abort_ev.is_set()):
    ...
    def encode_input(self) -> None:
        bdn = self.prepare()
        # Check abort immediately after BDN parsing
        _abort_ev = getattr(self, '_abort_event', None)
        if _abort_ev is not None and _abort_ev.is_set():
            logger.warning("Abort requested before encoding starts.")
            self._epochs = []
            return
        ...
        if _abort_ev is not None and _abort_ev.is_set():
            logger.warning("Abort requested after encoding, skipping verification and output.")
            self._epochs = []
            return
    ...
    def write_output(self) -> None:
        ...
        if is_pes:
            if EsMuiStream is None:
                logger.error("scenaristream library not available. Cannot write PES output. "
                             "Install with: pip install scenaristream")
            elif self.valid_stream:
