...
from typing import Union, Optional, Any, Type, Iterator
from dataclasses import dataclass, field

from opensup.utils.logging import LogFacility
from opensup.utils.bdvideo import BDVideo
from opensup.media.palette import PaletteEntry, Palette

logging = LogFacility.get_logger('OpenSUP')
...
class PCS(PGSegment):
    ...
    class PCSOff(Enum):
        ...
        PAL_ID     = 9
        N_OBJ_DEFS = 10
        LENGTH_SEG = N_OBJ_DEFS

    class CompositionState(IntEnum):
        NORMAL      = 0x00
        ACQUISITION = 0x40
        EPOCH_START = 0x80
        EPOCH_CONTINUE = ACQUISITION | EPOCH_START
...
class Epoch:
    ...
    def fetch(self) -> Iterator[DisplaySet]:
        for ds in self.ds:
            yield ds
