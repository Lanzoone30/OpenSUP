...
from opensup.core.segments import ODS, PDS
from opensup.utils.geometry import Shape, Box, MPEGTS_FREQ

from dataclasses import dataclass
try:
    from brule import Brule
except ImportError:
    Brule = None  # type: ignore
...
class PGraphics:
    ...
    def encode_rle(bitmap: npt.NDArray[np.uint8]) -> bytes:
        if Brule is None:
            raise ImportError("brule library required. Install: pip install brule")
...
class PGPalette(Palette):
    ...
    def store(self, palette: Union[dict[int, Any], Type['PGPalette'], Palette], pts: float) -> None:
